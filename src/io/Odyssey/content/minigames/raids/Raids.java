...
public class Raids {
    ...
    private static final int RAIDS_DAMAGE_FOR_REWARD = 250;
    ...
    public int currentHeight; // this is the problem
    /**
     * The current path
     */
    private int path;
...
// testing real quick
    ...
    public enum RaidRooms {
        STARTING_ROOM("start_room", 1, 0, new Location(3232, 5721)), OLM_ROOM("olm", 1, 0, new Location(3232, 5730)),
        STARTING_ROOM_2("start_room", 1, 1, new Location(3232, 5721)), OLM_ROOM_2("olm", 1, 1, new Location(3232, 5730));
...
//    public void startRaid(List<Player> players, boolean party) {   <<<<<<< this is code from the full working version of an old source (xeros)
...
//            p.heightLevel = RaidConstants.currentRaidHeight;
//        }
        currentHeight = RaidConstants.currentRaidHeight;
        RaidConstants.currentRaidHeight += 4;

        System.out.println("started raid currentHeight:"+currentHeight+", currentRaidHeight:" + RaidConstants.currentRaidHeight + ", ");
    ...
    public void killAllSpawns() {
        NPCHandler.kill(currentHeight, currentHeight,  7553, 7554, 7555);
    }
...
//			Server.getGlobalObjects().add(new GlobalObject(-1, 3232, 5749, currentHeight, 3, 10).setInstance(instance));
        ...
        if(killer != null) {
            int randomPoints = Misc.random(100000);
    ...
    public static final int OLM_HP = 1000;
    ...
    public void nextRoom(Player player) {
        ...
        player.getPA().movePlayer(roomPaths.get(playerRoom).getX(),
                ...
                roomPaths.get(playerRoom).getZ() == 1 ? currentHeight:currentHeight);
