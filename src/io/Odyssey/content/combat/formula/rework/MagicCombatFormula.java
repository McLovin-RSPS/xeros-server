...
import io.Odyssey.content.combat.effects.damageeffect.impl.amuletofthedamned.impl.AhrimEffect;
...
import io.Odyssey.model.SpellBook;
...
public class MagicCombatFormula implements CombatFormula {
    ...
    public int getMaxHit(Entity attacker, Entity defender, double specialAttackMultiplier,
        ...
        if (attacker.isPlayer()) {
            ...
            if (player.getSpellId() > -1 && !player.usingSpecial) {
                ...
                else if(!(player.wildLevel > 0) && player.getCombatItems().usingCrystalStaffSpecial())
                    hit = MagicMaxHit.getNightmareSpecialMaxHit(magicLevel, 120);
                else if(player.getCombatItems().usingCrystalStaffSpecial())
                    hit = MagicMaxHit.getNightmareSpecialMaxHit(magicLevel, 44);
                else if (this.hasSanguinestiCharges(player)) {
                    hit = this.getSanguinestuMaxHit(player);
                } else if (this.hasTumekensShadowCharges(player)) {
                    hit = this.getTumekensShadowMaxHit(player);
                } else if (this.hasAccursedCharges(player)) {
                    hit = this.getAccursedMaxHit(player);
        ...
        if (attacker.isPlayer()) {
            Player player = attacker.asPlayer();
            int magicLevel = player.playerLevel[Skill.MAGIC.getId()];
            if(player.getItems().isWearingItem(23900)){
                hit += 20;
            }
            ...
            if (player.getItems().isWearingItem(23900) &&
                    ...
                    player.getItems().isWearingItem(23848)) {
                hit += 8;
            }
    ...
    private static int[] TOXIC_TRIDENTS = {
            Items.TRIDENT_OF_THE_SWAMP,Items.CRYSTAL_STAFF_PERFECTED, Items.TRIDENT_OF_THE_SWAMP_E, Items.TRIDENT_OF_THE_SEAS_FULL
    };
    ...
    public boolean hasAccursedCharges(Player player) {
        if (player.getItems().hasAnyItem(Player.playerWeapon, Items.ACCURSED_SCEPTRE,
                Items.ACCURSED_SCEPTRE_U)) {
            return player.getSangStaffCharge() > 0;
        }
        return false;
    }
    public int getAccursedMaxHit(Player player) {
        int baseHit = 24;
        int magicLevel = Math.max(player.playerLevel[Skill.MAGIC.getId()], 75);
        int extraHits = (int) ((double) (magicLevel - 75) / 3);
        return baseHit + extraHits;
    }
    public boolean hasTumekensShadowCharges (Player player) {
        if (player.getItems().hasAnyItem(Player.playerWeapon, Items.TUMEKENS_SHADOW,
                Items.TUMEKENS_SHADOW_UNCHARGED)) {
            return player.getTumekensShadowCharge() > 0;
        }
        return false;
    }

    public int getTumekensShadowMaxHit(Player player) {
        int baseHit = 24;
        int magicLevel = Math.max(player.playerLevel[Skill.MAGIC.getId()], 75);
        int extraHits = (int) ((double) (magicLevel - 75) / 3);
        return baseHit + extraHits;
