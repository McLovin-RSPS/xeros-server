...
import static io.Odyssey.model.Items.TWISTED_BOW_L;
import static io.Odyssey.model.Items.TWISTED_BOW_P;
...
public class RangeMaxHit extends RangeData {
	...
	public static int getBestTwistedBowLMagicLevel(NPC npc) {
		NpcStats stats = NpcStats.forId(npc.getNpcId());
		return Math.max(stats.getMagicLevel(), stats.getMagic());
	}
	public static int getBestTwistedBowPMagicLevel(NPC npc) {
		NpcStats stats = NpcStats.forId(npc.getNpcId());
		return Math.max(stats.getMagicLevel(), stats.getMagic());
	}
	...
	public static double getTwistedBowLAccuracyBoost(int magicLevel) {
		if (magicLevel > 500)
			magicLevel = 500;
		double boost = 140 + ((3d * magicLevel - 10d) / 100d) - (Math.pow(3d * magicLevel / 10d - 100d, 2) / 100d);
		return (Math.min(boost, 140) / 100);
	}
	public static double getTwistedBowPAccuracyBoost(int magicLevel) {
		if (magicLevel > 500)
			magicLevel = 500;
		double boost = 140 + ((3d * magicLevel - 10d) / 100d) - (Math.pow(3d * magicLevel / 10d - 100d, 2) / 100d);
		return (Math.min(boost, 140) / 100);
	...
	public static int calculateRangeAttack(Player c) { // its not evne being used kkk
		...
		if (c.npcAttackingIndex > 0 && c.getItems().isWearingItem(Items.TWISTED_BOW) || c.getItems().isWearingItem(Items.TWISTED_BOW_L) || c.getItems().isWearingItem(TWISTED_BOW_P)) {
	...
	public static int maxHit(Player c) {
		...
		} else if (c.playerEquipment[Player.playerWeapon] == Items.TOXIC_BLOWPIPE) {
		...
		} else if (c.getCombatItems().wearingWebweaverBow() || c.usingOtherRangeWeapons) {
			rangedStrength = RangeData.getRangeStr(c, Player.playerArrows);
		} else {
		...
		boolean hasDarkVersion = (c.petSummonId == 30116 || c.petSummonId == 30120 || c.petSummonId == 29966);
