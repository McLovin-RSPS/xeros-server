...
//		}
		...
		if (i == 27) {
			if (!c.rigour && !Server.isDebug() && !Boundary.isIn(c, Boundary.LONE_SURVIVER) && !Boundary.isIn(c, Boundary.LUMBRIDGE_OUTLAST_AREA) && !Boundary.isIn(c, Boundary.LUMBRIDGE_OUTLAST_LOBBY)) {
				c.getPA().sendConfig(PRAYER_GLOW[i], 0);
		...
		if (i == 28) {
			if (!c.augury && !Server.isDebug() && !Boundary.isIn(c, Boundary.LONE_SURVIVER) && !Boundary.isIn(c, Boundary.LUMBRIDGE_OUTLAST_AREA) && !Boundary.isIn(c, Boundary.LUMBRIDGE_OUTLAST_LOBBY)) {
				c.getPA().sendConfig(PRAYER_GLOW[i], 0);
