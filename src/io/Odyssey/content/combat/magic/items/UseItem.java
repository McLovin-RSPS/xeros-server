...
import io.Odyssey.content.PlatinumTokens;
...
import io.Odyssey.content.item.lootable.impl.CrystalChest;
import io.Odyssey.content.item.lootable.impl.FoeMysteryBox;
import io.Odyssey.content.item.lootable.impl.HunllefChest;
...
import io.Odyssey.content.skills.firemake.Firemaking;
import io.Odyssey.content.skills.herblore.*;
...
import io.Odyssey.content.trails.MasterClue;
...
import io.Odyssey.util.logging.player.ItemOnItemLog;
...
public class UseItem {
	...
	public static void ItemonObject(Player c, int objectID, int objectX, int objectY, int itemId) {
		...
		switch (objectID) {
			case 37341:
				...
				c.getDH().sendStatement("@red@You need F's key to unlock this chest.");
				break;
			case 33311:
				c.objectYOffset = 5;
				c.objectXOffset = 5;
	...
	public static void ItemonItem(final Player c, final int itemUsed, final int useWith, final int itemUsedSlot, final int usedWithSlot) {
		...
		if (SanguinestiStaff.useItem(c, itemUsed, useWith))
			return;
		if (TumekensShadow.useItem(c, itemUsed, useWith)) {
			return;
		}
		...
		if (itemUsed == 554 || itemUsed == 562 || itemUsed == 565 && useWith == 23900) {
			if (c.crystalStaff > 1) {
				c.sendMessage("Your staff still has charge");
				return;
			}
			if (c.getItems().playerHasItem(554, 20000) &&
					c.getItems().playerHasItem(562, 10000) && c.getItems().playerHasItem(565, 5000)) {
				c.staffIncrease(+5000);
				c.getItems().deleteItem(554, 20000);
				c.getItems().deleteItem(562, 10000);
				c.getItems().deleteItem(565, 5000);
				c.sendMessage("You now have " + c.crystalStaff + " Charge in your staff");

			} else {
				c.sendMessage("You dont have required items for full charge, 5k bloods, 10k chaos runes and 20k fire runes");
				return;
			}
		}
		if (itemUsed == 23877 && useWith == 20201) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
			}
			c.crystaIncrease(+cshardamount);
			c.getItems().deleteItem(itemUsed, cshardamount);
			c.sendMessage("You now have " + c.crystalCharge + " Crystal shard in your Bow");
			return;
		}
		if (itemUsed == 23877 && useWith == 25867) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 23877 && useWith == 25884) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 11212 && useWith == 29988) {
			int cshardamount = c.getItems().getItemAmount(11212);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
			}
		...
		if (itemUsed == 11212 && useWith == 29985) {
			int cshardamount = c.getItems().getItemAmount(11212);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 23877 && useWith == 25886) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 23877 && useWith == 25888) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 23877 && useWith == 25890) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 23877 && useWith == 25892) {
			int cshardamount = c.getItems().getItemAmount(23877);
			int maxCshardToAdd = 10000 - c.crystalCharge;
			if ((c.crystalCharge + cshardamount) >= 10000) {
				cshardamount = maxCshardToAdd;
		...
		if (itemUsed == 946 && useWith == 6573 || useWith == 6573 && itemUsed == 946) { //creating ammy of fury
			if (c.getItems().playerHasItem(946) && c.getItems().playerHasItem(6573)) {
					c.getItems().deleteItem2(6573, 1);
					c.getItems().deleteItem2(1737, 1);
					c.getItems().addItem(6585, 1);
					c.getPA().addSkillXPMultiplied(870, Player.playerCrafting, true);
					c.sendMessage("You create a amulet of fury");
					c.nextChat = -1;
				} else {
					c.sendMessage("You need a knife and a onyx to create a amulet of fury");
				}
			}
		...
		if (itemUsed == 26421 && useWith == 4151 || useWith == 26421 && itemUsed == 4151) { //dragon huner lance

			if (c.getItems().playerHasItem(26421) && c.getItems().playerHasItem(4151)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26482, 1);
				c.getDH().sendStatement("You have created a bad ass whip.");
				c.nextChat = -1;
			}
		}

		if (itemUsed == 26807 && useWith == 26852 || useWith == 26807 && itemUsed == 26852) { //green dye with robe top of eye

			if (c.getItems().playerHasItem(26807) && c.getItems().playerHasItem(26852)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26866, 1);
				c.getDH().sendStatement("You dye your robe of the eye");

			}

		}

		if (itemUsed == 25742 && useWith == 22325 || useWith == 25742 && itemUsed == 22325) { //using holy kit with scythe of vitur

			if (c.getItems().playerHasItem(25742) && c.getItems().playerHasItem(22325)) {
				c.getItems().hasItemOnOrInventory(Items.CHISEL);
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(25736, 1);
				c.getDH().sendStatement("You combine a holy ornament kit with your scythe of vitur");

			}
		}

	if (itemUsed == 1755 && useWith == 25736 || useWith == 25736 && itemUsed == 1755) { //using chisel to remove scythe kit

		if (c.getItems().playerHasItem(1755) && c.getItems().playerHasItem(25736)) {
			c.getItems().hasItemOnOrInventory(Items.CHISEL);
			c.getItems().deleteItem2(useWith, 1);
			c.getItems().addItem(22325, 1);
			c.getItems().addItem(25742, 1);
			c.getDH().sendStatement("You remove a holy ornament kit from your scythe");
			c.nextChat = -1;
		}
		...
		if (itemUsed == 26378 && useWith == 11832 || useWith == 26378 && itemUsed == 11832) { //dragon huner lance
			if(c.playerLevel[12] < 95) {
				c.sendMessage("You must have a crafting level of at least 95 to create Torva.");
				return;
			}
			if (c.getItems().playerHasItem(26378) && c.getItems().playerHasItem(11832)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26384, 1);
				c.getDH().sendStatement("@blu@You have created Torva!");
				c.nextChat = -1;
			}
		}

		if (itemUsed == 26380 && useWith == 11834 || useWith == 26380 && itemUsed == 11834) { //dragon huner lance
			if(c.playerLevel[12] < 95) {
				c.sendMessage("You must have a crafting level of at least 95 to create Torva.");
				return;
			}
			if (c.getItems().playerHasItem(26380) && c.getItems().playerHasItem(11834)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
		...
		if (itemUsed == 26376 && useWith == 11832 || useWith == 2637 && itemUsed == 11832) { //dragon huner lance
			if(c.playerLevel[12] < 95) {
				c.sendMessage("You must have a crafting level of at least 95 to create Torva.");
				return;
			}
		...
		if (itemUsed == 26421 && useWith == 13072 || useWith == 26421 && itemUsed == 13072) { //dragon huner lance
			if (c.getItems().playerHasItem(26421) && c.getItems().playerHasItem(13072)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26469, 1);
				c.getDH().sendStatement("You have created a bad ass elite void top.");
				c.nextChat = -1;
			}
		}

		if (itemUsed == 26421 && useWith == 13073 || useWith == 26421 && itemUsed == 13073) { //dragon huner lance
			if (c.getItems().playerHasItem(26421) && c.getItems().playerHasItem(13073)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26471, 1);
				c.getDH().sendStatement("You have created bad ass elite void bottoms.");
				c.nextChat = -1;
			}
		}

		if (itemUsed == 26421 && useWith == 11663 || useWith == 26421 && itemUsed == 11663) { //dragon huner lance
			if (c.getItems().playerHasItem(26421) && c.getItems().playerHasItem(11663)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26473, 1);
				c.getDH().sendStatement("You have created a bad ass void mage helm.");
				c.nextChat = -1;
			}
		}

		if (itemUsed == 26421 && useWith == 11664 || useWith == 26421 && itemUsed == 11664) { //dragon huner lance
			if (c.getItems().playerHasItem(26421) && c.getItems().playerHasItem(11664)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26475, 1);
				c.getDH().sendStatement("You have created a bad ass void ranger helm.");
				c.nextChat = -1;
			}
		}

		if (itemUsed == 26421 && useWith == 11665 || useWith == 26421 && itemUsed == 11665) { //dragon huner lance
			if (c.getItems().playerHasItem(26421) && c.getItems().playerHasItem(11665)) {
				c.getItems().deleteItem2(itemUsed, 1);
				c.getItems().deleteItem2(useWith, 1);
				c.getItems().addItem(26477, 1);
				c.getDH().sendStatement("You have created a bad ass void melee helm.");
				c.nextChat = -1;
			}
		}
		...
		if((itemUsed == 1755 && useWith == 26879)) {//fragments
			if(c.playerLevel[12] < 75) {
				c.sendMessage("You must have a crafting level of at least 75 to chisel fragments!");
				return;
			}
				c.startAnimation(886);
			if (c.getItems().playerHasItem(1755, 1)&& c.getItems().playerHasItem(26879, 1)) {
				c.getItems().deleteItem(26879, 1);
				c.getItems().addItem(26878, 3);
				c.sendMessage("You make some guardian fragments");
				c.getPA().addSkillXPMultiplied(75, Player.playerCrafting, true);
				return;
			}
			c.startAnimation(886);
			if (c.getItems().playerHasItem(26879, 1)&& c.getItems().playerHasItem(1755, 1)) {
				c.getItems().deleteItem(26879, 1);
				c.getItems().addItem(26878, 3);
				c.sendMessage("You make some guardian fragments");
				c.getPA().addSkillXPMultiplied(75, Player.playerCrafting, true);
			} else {
				c.sendMessage("You need a chisel and guardian essence make fragments");
				return;
			}
		}
