package io.Odyssey.content.combat.pvp;
...
public class PkpRewards {
    ...
    public static void award(Player dying, Player killer) {
        ...
        if (!Boundary.isIn(killer, Boundary.LONE_SURVIVER_AREA)) {
            DiscordMessager.sendLogs("[Kill]" + killer.getDisplayName() + " killed " + dying.getDisplayName() + " at" + killer.absX + ", " + killer.absY);
        }
...
//                    && killer.getBH().hasTarget() && killer.getBH().getTarget().getName().equalsIgnoreCase(dying.playerName)) {
        ...
        if (Boundary.isIn(dying, Boundary.WILDERNESS_PARAMETERS)) {
            ...
            if (canReceiveRewards) {
                ...
                int random = 30;
                killer.pkp += random;
                    if (dying.amDonated >= 25 && dying.amDonated <= 99) { //regular donator
                        int bonuspkp = 3;
                        killer.pkp += bonuspkp;
                    ...
                    } else if (killer.amDonated >= 250 && killer.amDonated <= 999) { //diamond club
                        int bonuspkp = 10;
                        killer.pkp += bonuspkp;
                        killer.sendMessage("[@red@PKP@bla@] You are rewarded an extra@red@ " + bonuspkp + " pkp @bla@for your donator rank.");
