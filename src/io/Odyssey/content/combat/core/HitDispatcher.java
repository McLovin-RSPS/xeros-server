...
import io.Odyssey.content.combat.effects.damageeffect.impl.ToxicBlowpipeEffect;
import io.Odyssey.content.combat.effects.damageeffect.impl.ToxicStaffOfTheDeadEffect;
import io.Odyssey.content.combat.effects.damageeffect.impl.amuletofthedamned.impl.GuthanEffect;
import io.Odyssey.content.combat.effects.special.impl.HolyScytheOfVitur;
...
import io.Odyssey.content.combat.specials.impl.*;
...
public abstract class HitDispatcher {
    ...
    private void playerHitEntity(CombatType combatType, Special special, boolean applyingMultiHitAttack) {
        ...
        if (combatType.equals(CombatType.MELEE)) {
            ...
            int damage1 = damage;
            if (attacker.get2XDamage()) {
                damage *= 2;
                attacker.sendMessage(" Melee You're receiving 2x damage. damage1:"+damage1+", 2x damage:"+damage);
            }
            ...
            if (maximumDamage > 0) {
                if (Misc.trueRand(4) == 1) {
                    if (EquipmentSet.NEWARMA.isWearing(attacker)) {
                        int maximumHealthCap = attacker.getHealth().getMaximumHealth() + 10;
                        attacker.getHealth().increase(maximumDamage, maximumHealthCap);
                        defender.startGraphic(new Graphic(399));
            ...
            if (attacker.get2XDamage()) {
                damage *= 4;
                attacker.sendMessage("You're receiving 2x damage.");
            }
            ...
            if (attacker.weaponUsedOnAttack == 11235 || attacker.weaponUsedOnAttack == 30051 || attacker.weaponUsedOnAttack == 27655 || attacker.weaponUsedOnAttack == 12765 || attacker.weaponUsedOnAttack == 12766 || attacker.weaponUsedOnAttack == 12767
            ...
            if (attacker.weaponUsedOnAttack == 8818 && attacker.usingSpecial) {
            ...
            if (attacker.weaponUsedOnAttack == 26374 && attacker.usingSpecial) {
    ...
    private void dropArrows() {
        ...
        if (weaponId == Items.CRAWS_BOW||weaponId == Items.WEBWEAVER_BOW) {
            return;
        }
    ...
    private void dropArrow(int arrowId) { // TODO delete arrows from player
        if (Boundary.LONE_SURVIVER.in(attacker))
            return;
