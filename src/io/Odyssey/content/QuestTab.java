...
import io.Odyssey.content.achievement.inter.TasksInterface;
...
import io.Odyssey.model.Area;
import io.Odyssey.model.SquareArea;
...
public class QuestTab {
    ...
    public void updateInformationTab() {
        ...
        if (player.getRights().contains(Right.OWNER)) {
            player.getPA().sendFrame126("@or1@- Players: @gre@" + PlayerHandler.getPlayerCount(), lines.get(index++));
        } else {
        ...
        for (int index = 0; index < COIN_TAB_BUTTONS.length; index++) {
            if (button == COIN_TAB_BUTTONS[index]) {
                ...
                switch(coinTab) {
                    ...
                    case PRESETS:
                        Area[] areas = {
                            new SquareArea(3066, 3521, 3289, 3428),
                        };
