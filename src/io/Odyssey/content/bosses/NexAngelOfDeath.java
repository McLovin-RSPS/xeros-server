...
public class NexAngelOfDeath extends NPC {
    ...
    public static final Position[] chargePosition = {new Position(3744, 3987), new Position(3769, 3987),
            new Position(3757, 3975), new Position(3757, 3999)};
    ...
    @Override
    public void process() {
        PlayerHandler.nonNullStream().filter(p -> Boundary.isIn(p, Boundary.NexAngelOfDeath))
                .forEach(p -> {
    ...
    public static void rewardPlayers(boolean eventCompleted) {
        ...
        PlayerHandler.nonNullStream().filter(p -> Boundary.isIn(p, Boundary.angelOfDeathWorldevent)).forEach(p -> {
            if (!eventCompleted) {
