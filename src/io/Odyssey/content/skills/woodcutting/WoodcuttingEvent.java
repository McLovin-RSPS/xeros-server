...
public class WoodcuttingEvent extends Event<Player> {
	...
	public void execute() {
		...
		if (tree.equals(Tree.AVATAR_OF_CREATION)) {
			int randomTele = 1;
			if (attachment.getItems().playerHasItem(AvatarOfCreation.KEY)) {
				attachment.moveTo(new Position(3072 + randomTele, 3505 + randomTele));
				AvatarOfCreation.deleteEventItems(attachment);
				return;
			}
			int randomTele2 = Misc.random(2);
			attachment.canLeaveAvatarOfCreation = true;
	...
	private void handleRewards() {
		...
		int give = Misc.random(1000);
		if (give > 247 && give < 249 &&
				!attachment.getItems().hasAnyItem(10941) && !attachment.getItems().hasItemOnOrInventory(10941)
				&& !attachment.getItems().bankContains(10941)) {
			attachment.sendMessage("You notice a lumberjack piece falling from the tree and pick it up.");
			attachment.getItems().addItemUnderAnyCircumstance(10941, 1);


		} else if (give > 447 && give < 449 &&
				!attachment.getItems().hasAnyItem(10940) && !attachment.getItems().hasItemOnOrInventory(10940)
				&& !attachment.getItems().bankContains(10940)) {
			attachment.sendMessage("You notice a lumberjack piece falling from the tree and pick it up.");
			attachment.getItems().addItemUnderAnyCircumstance(10940, 1);

		} else if (give > 667 && give < 669 &&
				!attachment.getItems().hasAnyItem(10933) && !attachment.getItems().hasItemOnOrInventory(10933)
				&& !attachment.getItems().bankContains(10933)) {
			attachment.sendMessage("You notice a lumberjack piece falling from the tree and pick it up.");
			attachment.getItems().addItemUnderAnyCircumstance(10933, 1);
		} else if (give > 887 && give < 779 &&
				!attachment.getItems().hasAnyItem(10939) && !attachment.getItems().hasItemOnOrInventory(10939)
				&& !attachment.getItems().bankContains(10939)) {
			attachment.sendMessage("You notice a lumberjack piece falling from the tree and pick it up.");
			attachment.getItems().addItemUnderAnyCircumstance(10939, 1);
