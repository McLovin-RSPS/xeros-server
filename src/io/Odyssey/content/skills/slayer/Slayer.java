...
public class Slayer {
	...
	public static int[] SUPERIOR_COUNTERPARTS = { 448, 406, 414, 7272, 421, 419, 435, 417, 446, 484, 7276, 437, 7277, 3209, 6, 7279, 423, 411, 498, 1543, 4005, 415, 11, 11278 };
...
//			}
			...
			if (isTaskNpc(npc)) {
				...
				master.ifPresent(m -> {
					switch (m.getId()) {
						case 8623:
							if (KonarSlayer.checkLocation(player)) {
								...
								if (AvatarOfCreation.activeCelastrusSeed) {
									rewardAmount = 2;
								}
					...
					if (taskAmount == 0) {
						...
						if (AvatarOfCreation.activeNoxiferSeed) {
							multiplier = 2;
						}
						...
						if (consecutiveTasks == 10) {
							...
							} else if (player.amDonated >= 25 && player.amDonated < 100) {
								points += 3;
							...
							} else if (player.amDonated >= 250 && player.amDonated < 1000) {
								points += 10;
	...
	public int getBlockTaskCost() {
		if (player.amDonated >= 5 && player.amDonated <= 49) {
			return 80;
		} else if (player.amDonated >= 25 && player.amDonated <= 99) {
			return 80;
			...
			return 60;
		} else if (player.amDonated >= 250 && player.amDonated <= 999) {
			return 50;
		} else if (player.amDonated >= 1000) {
			return 40;
		}
