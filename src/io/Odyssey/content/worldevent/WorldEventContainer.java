...
import io.Odyssey.content.worldevent.impl.AngelofdEathWorldEvent;
...
public class WorldEventContainer {
    ...
    public static final List<WorldEvent> WORLD_EVENT_LIST = Collections.unmodifiableList(Lists.newArrayList(new TournamentWorldEvent(), new AngelofdEathWorldEvent(), new AvatarOfCreationWorldEvent(), new WildernessBossWorldEvent()));
    ...
    public String getTimeUntilWildEvent() {
        return getTimeUntilEvent(new WildernessBossWorldEvent());
    }

    public String getTimeUntilLoneSurvivor() {
        return getTimeUntilEvent(new TournamentWorldEvent());
    }

    public String getCurrentActiveEvent() {
        Optional<WorldEvent> event = getCurrentEvent();
        if (event.isPresent()) {
            return event.get().getEventName();
        } else {
            return "None";
        }
    }
