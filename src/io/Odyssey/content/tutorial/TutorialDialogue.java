...
public class TutorialDialogue extends DialogueBuilder {
    ...
    public static final int TUTORIAL_NPC = 11276;
    ...
    public static void finish(Player player, ModeType modeType) {
        ...
        PlayerHandler.executeGlobalMessage("[@pur@New Player@bla@] " + player.getDisplayNameFormatted() + " @bla@has logged in! Welcome!");
        DiscordMessager.sendNewPlayers("[New Player] " + player.getDisplayName() + " has logged in! Welcome!");
    }
    ...
    public TutorialDialogue(Player player, boolean repeat) {
        ...
        setNpcId(TUTORIAL_NPC);
        if (!Server.isTest()) {
            npc(new Position(3086, 3492), "Welcome to " + Configuration.SERVER_NAME + "!", "Here is our home area!", "Don't forgot to join our ::discord.");
            npc(new Position(3089, 3478), "Here you can find all the shops needed", "when you first start out! You can buy combat gear,", "foods and pots, or show off your fashion skills!");
            npc(new Position(3102, 3499), "This is the LMS Portal.", "Anybody can join, any level or game mode!", "Use the Quest Tab to see when the next", "event will happen!");
            npc(new Position(3078, 3494), "Here you can get Slayer tasks");
            npc(new Position(3094, 3510), "If you decide to be a restricted game mode", "you can use the shops here!", "Including a UIM Storage chest!");
            npc(new Position(3092, 3486 ), "Vote store,Donations,Ticket exchange, Giveaways!");
        }
        if (!repeat) {// you changed edge here? i pasted al the files here
