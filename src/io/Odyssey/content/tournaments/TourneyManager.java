...
import io.Odyssey.sql.outlast.OutlastRecentWinnersAdd;
...
public class TourneyManager {
    ...
    private boolean clearCurrentArena(Player player) {
        if (isArenaActive()) {
            if (Boundary.getPlayersInBoundary(Boundary.LUMBRIDGE_OUTLAST_AREA) == 0) {
                player.sendMessage("Killing arena, 0 entites found inside Lone Survivor but Lone Survivor @red@WAS@bla@ still active.");
                victors.clear();
                arenaActive = false;
                return true;
            } else {
    ...
    public static boolean handleObjects(Player player, WorldObject object, int optionId) {
        if (object == null || object.getId() != 41808)
            return false;
        ...
        switch (optionId) {
            ...
            case 2:
                ...
                String msg = TourneyManager.getSingleton().isLobbyOpen() ? "There are currently " + players + " in the lobby." : TourneyManager.getSingleton().isArenaActive() ? "There is " + players + " remaining in the fight!" : "The Lone Surviver event hasn't started yet. It will begin in another: " + TourneyManager.getSingleton().getTimeRemaining();
                player.sendMessage(msg);
                return true;

            case 3:
                player.sendMessage(TourneyManager.getSingleton().isLobbyOpen() ? "The tournament lobby is open and will start shortly." : TourneyManager.getSingleton().isArenaActive() ? "The Lone Surviver tournament has already begun." : "The next Lone Surviver tournament will begin in: " + TourneyManager.getSingleton().getTimeRemaining());
                return true;
        }
        return false;
    }
    ...
    private void beginTournament() {
        ...
        CycleEventHandler.getSingleton().addEvent(ARENA_TASK_OBJECT, new CycleEvent() {
            @Override
            public void execute(CycleEventContainer container) {
                if (currentPlayers.isEmpty() || Boundary.getPlayersInBoundary(Boundary.LONE_SURVIVER) == 0) {
                    endGame();
                    container.stop();
                }
            }
    ...
    public void leaveLobby(Player player, boolean xlog) {
        if (!player.spectatingTournament && isParticipant(player)) {
            ...
            if (xlog)
                player.getPA().forceMove(3146, 3480, 0, true);
            else
                player.getPA().movePlayer(new Coordinate(3146, 3480));
            ...
            if (player != null) {
                ...
                if (i == 2) {
                    ...
                    if (player.streak == 5) {
                        ...
                        announce("@blu@" + name + " has just reached 5 wins in a row in Lone Surviver!");
                        ...
                        announce("@blu@" + name + " has just reached 10 wins in a row in Lone Surviver!");
    ...
    public String getTimeLeft() {
        if (isArenaActive()) {
            return "Lone Surviver: @gre@Active";
        } else if (isLobbyOpen()) {
            return "Lone Surviver: @whi@" + getLobbyTime();
        } else {
            return "Lone Surviver: @red@" + WorldEventContainer.getInstance().getTimeUntilEvent(new TournamentWorldEvent()) + " minutes";
        }
    }
