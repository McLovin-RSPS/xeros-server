package io.Odyssey.model.collisionmap;
...
public class Region {
    ...
    private static final ArrayList<WorldObject> worldObjects = new ArrayList<>();
...
//        region.worldObjects.addAll(worldObjects); // need to fix this as causing crashes
...
//	}
    ...
    public static void load() {
        try {
            ...
            verifyCustomMaps();
            File f = new File(Server.getDataDirectory() + "/mapdata/map_index.dat");
            ...
            for (int i = 0; i < size; i++) {
                byte[] file1 = getBuffer(new File(Server.getDataDirectory() + "/mapdata/map/" + mapObjectsFileIds[i] + ".gz"));
                byte[] file2 = getBuffer(new File(Server.getDataDirectory() + "/mapdata/map/" + mapGroundFileIds[i] + ".gz"));
            ...
            long totalObjects = worldObjects.stream().count();
     ...
     */
    private  HashMap<Integer, ArrayList<WorldObject>> worldObjects2 = new HashMap<>();
    ...
    private static void loadMap(RegionData regionData) {
        try {
            ...
            if (file1 == null) {
                file1 = getBuffer(new File(Server.getDataDirectory() + "/mapdata/map/" + regionData.getObjects() + ".gz"));
            } else {
                customMapFiles++;
            }
            if (file2 == null) {
                file2 = getBuffer(new File(Server.getDataDirectory() + "/mapdata/map/" + regionData.getLandscape() + ".gz"));
            } else {
                customMapFiles++;
            }
            if (file1 == null || file2 == null) {
 ...
 // raids 3
    new WorldObject(46164, 3539, 5146, 2, 0),
            // Abyss
