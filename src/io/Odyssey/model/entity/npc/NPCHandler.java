...
import io.Odyssey.content.achievement_diary.AchievementDiaryKills;
...
import io.Odyssey.content.minigames.fight_cave.Wave;
import io.Odyssey.content.minigames.inferno.InfernoWaveData;
import io.Odyssey.content.minigames.pest_control.PestControl;
import io.Odyssey.content.minigames.tokkul_pit.Wave1;
import io.Odyssey.content.minigames.xeric.XericWave;
...
import io.Odyssey.model.entity.npc.data.AttackAnimation;
...
public class NPCHandler {
    ...
    public static boolean isFightCaveNpc(NPC npc) {
        ...
        switch (npc.getNpcId()) {
        case io.Odyssey.content.minigames.fight_cave.Wave.TZ_KEK_SPAWN: 
        case io.Odyssey.content.minigames.fight_cave.Wave.TZ_KIH: 
        case io.Odyssey.content.minigames.fight_cave.Wave.TZ_KEK: 
        case io.Odyssey.content.minigames.fight_cave.Wave.TOK_XIL: 
        case io.Odyssey.content.minigames.fight_cave.Wave.YT_MEJKOT: 
        case io.Odyssey.content.minigames.fight_cave.Wave.KET_ZEK: 
        case Wave.TZTOK_JAD: 
            return true;
        }
        return false;
    }
    public static boolean isTokkulPit1Npc(NPC npc) {
        if (npc == null) return false;
        switch (npc.getNpcId()) {
            case io.Odyssey.content.minigames.tokkul_pit.Wave1.OBSIDIAN_RAPTORL1:
            case io.Odyssey.content.minigames.tokkul_pit.Wave1.TOK_XIL1:
            case io.Odyssey.content.minigames.tokkul_pit.Wave1.OBSIDIAN_MIDGIT:
            case io.Odyssey.content.minigames.tokkul_pit.Wave1.OBSIDIAN_RAPTOR:
            case io.Odyssey.content.minigames.tokkul_pit.Wave1.JADS_OTHER_BUDDY:
            case io.Odyssey.content.minigames.tokkul_pit.Wave1.JADS_BUDDY:
            case Wave1.FOURTH_AGE_JAD:
                return true;
        }
        return false;
    }
            ...
            if (PlayerHandler.players[j] != null) {
                ...
                if (PlayerHandler.players[j].goodDistance(c.absX, c.absY, npc.absX, npc.absY, multiAttackDistance(npc))) {
                    ...
                    if (npc.getAttackType() == CombatType.SPECIAL) {
                        ...
                        if (npc.getNpcId() == 11994 || npc.getNpcId() == 11993) {
                        ...
                        if (npc.getNpcId() == 11993 || npc.getNpcId() == 11994) {
                            if (vetionSpellCoordinates.stream().noneMatch(p -> p[0] == c.absX && p[3] == c.absY)) {
                                continue;
                            }
                        }
    ...
    void Tokkulpit1DeathHandler(Player player, NPC npc) {
        if (npc != null) {
            if (player != null) {
                if (player.getTokkulPit1() != null) {
                    if (isTokkulPit1Npc(npc)) killedTokkulPit1(player);
                }
            }
        }
    }
    ...
    private void killedTokkulPit1(Player player) {
        if (player.getTokkulPit1() != null) {
            player.getTokkulPit1().setKillsRemaining(player.getTokkulPit1().getKillsRemaining() - 1);
            if (player.getTokkulPit1().getKillsRemaining() <= 0) {
                player.waveId1++;
                player.getTokkulPit1().spawn();
            }
        }
    }
    ...
    public int followDistance(NPC npc) {
        if (Boundary.isIn(npc, Boundary.GODWARS_BOSSROOMS) || Boundary.isIn(npc, Boundary.CORPOREAL_BEAST_LAIR) || Boundary.isIn(npc, Boundary.CERBERUS_BOSSROOMS) || Boundary.isIn(npc, Boundary.NexAngelOfDeath))
            ...
            if (hasDistance) {
                ...
                if (c.respawnTimer <= 0) {
                    ...
                    if (npc.getNpcId() == 10402) {
                        ColossalHydra.attack(npc);
                    }
                    // handle NexAngelOfDeath attacks
                    if (npc.getNpcId() == 11299) {
                        NexAngelOfDeath.attack(npc);
                    }
                    if (npc.getNpcId() == 11670) {
                        NexAngelOfDeathWE.attack(npc);
                    }
                    npc.attackTimer = getNpcDelay(npc);
    ...
    public int getMaxHit(Player c, NPC npc) {
        if (npc == null) {
            c.sendMessage("this npc is null");
            return 0;
        }
        if (Boundary.isIn(npc, Boundary.XERIC)) {
        ...
        case 11402:
            case 11815:
            case 11814:
            return npc.getAttackType() == CombatType.MELEE ? 47 :npc.getAttackType() == CombatType.RANGE ? 50:  npc.getAttackType() == CombatType.SPECIAL ? 49 : 30;
        case 9021: 
        //melee
        case 9022: 
        //range
