...
import io.Odyssey.content.combat.formula.RangeMaxHit;
...
public class NPCHitPlayer {
    ...
    public static void applyDamage(NPC npc, NPCHandler context) {
        if (npc != null) {
            ...
            if (c.attackTimer <= 3) {
                if (!NPCHandler.isTokkulPit1Npc(npc) || npc.getNpcId() != 319) {
                    c.startAnimation(MeleeData.getBlockEmote(c));
                }
            }
            if (npc.getNpcId()>= 7931 && npc.getNpcId() <= 7940) {
          ...
          /*  if (c.getItems().isWearingItem(23900)) {
            ...
            if(c.getItems().isWearingItem(20201)){
                c.crystalCharge -= 1;

            }
            if (c.crystalCharge < 1 && c.getItems().isWearingItem(20201)){
                c.getItems().unequipItem(20201,3);
            ...
            if (c.respawnTimer <= 0) {
                            ...
                            if (c.protectingMelee() && !protectionIgnored) {
                                if (npc.getNpcId() == 5890)
                                    damage /= 3;
                                        ...
                                        || npc.getNpcId() == 320 || npc.getNpcId() == 6615 || npc.getNpcId() == 10402 ||npc.getNpcId() == 11670 || npc.getNpcId() == 11299 || npc.getNpcId() == 5916 || npc.getNpcId() == 11246  || npc.getNpcId() == 8781
                ...
                if (npc.getNpcId() == 10402 || npc.getNpcId() == 10402 || npc.getNpcId() == 10402 || npc.getNpcId() == 10402 ||npc.getNpcId() == 10402 || npc.getNpcId() == 10402 || npc.getNpcId() == 10402 || npc.getNpcId() == 10402) {
                    ...
                    if (npc.getNpcId() == 11294 || npc.getNpcId() == 11670 || npc.getNpcId() == 11299) {
                        int distanceFromTarget = c.distanceToPoint(npc.getX(), npc.getY());
