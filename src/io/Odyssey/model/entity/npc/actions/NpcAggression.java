...
public class NpcAggression {
    ...
    public static int getCloseRandomPlayer(NPC npc) {
        ...
        for (int j = 0; j < PlayerHandler.players.length; j++) {
            if (PlayerHandler.players[j] != null) {
                ...
                if (Server.npcHandler.goodDistance(PlayerHandler.players[j].absX, PlayerHandler.players[j].absY, npc.absX, npc.absY, distanceRequired) || NPCHandler.isFightCaveNpc(npc)||NPCHandler.isFightCaveNpc(npc)) {
    ...
    private static int getAggressionDistance(NPC npc, Player player, int baseDistance) {
        ...
        if (Boundary.isIn(npc, Boundary.SARACHNIS_LAIR) || Boundary.isIn(npc, Boundary.MIMIC_LAIR) || Boundary.isIn(npc, Boundary.GROTESQUE_LAIR) || Boundary.isIn(npc, Boundary.NexAngelOfDeath)) {
            return 25;
        }
        if (npc.getNpcId() > 0)
            switch (npc.getNpcId()) {
    ...
    public static boolean isAggressive(NPC npc) {
        ...
        if (Boundary.isIn(npc, Boundary.GODWARS_BOSSROOMS)
                ...
                || Boundary.isIn(npc, Boundary.RAIDS3ROOMS)
                || Boundary.isIn(npc, Boundary.AvatarOfCreation)
                || Boundary.isIn(npc, Boundary.NexAngelOfDeath)
                || Boundary.isIn(npc, Boundary.REVENANT_MALEDICTUS) && npc.getBehaviour().isAggressive()
                || Boundary.isIn(npc, Boundary.SAND_CRABS) && npc.getBehaviour().isAggressive()
        ...
        switch (npc.getNpcId()) {
            ...
            case Npcs.GREATER_SKELETON_HELLHOUND:
            case Npcs.SKELETON_HELLHOUNDZ:
            //case 6611:
           // case 6612:
            case 11993:
            case 11994:
        ...
        if (npc.inXeric() && npc.getHealth().getMaximumHealth() > 0)
            return true;
        return NPCHandler.isFightCaveNpc(npc) || NPCHandler.isTokkulPit1Npc(npc) ;
    }
