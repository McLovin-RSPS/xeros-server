...
import static io.Odyssey.model.entity.npc.NPCHandler.*;
        ...
        switch (npc.getNpcId()) {
            ...
            case 11299:
                ...
                if (chance > 1 && chance < 9) {
                    ...
                    int endGfx = -1;
                    int projectileId = -1;
                    if (NexAngelOfDeath.phase == 0) {
                        projectileId = 1997;
                        endGfx = 1998;
                    } else if (NexAngelOfDeath.phase == 1) {
                        projectileId = 2002;
                        endGfx = 2003;
                    } else if (NexAngelOfDeath.phase == 2) {
                        projectileId = 2007;    /// No I didnt add it to spawns, well I did but removed it did you restart? Yea, server could be hung up again
                        endGfx = 2008;
                    }
                    npc.endGfx = endGfx;
                    npc.projectileId = projectileId;
            ...
            case 11670:
                ...
                if (chance > 1 && chance < 9) {
                    ...
                    int endGfx = -1;
                    int projectileId = -1;
                    if (NexAngelOfDeathWE.phase == 0) {
                        projectileId = 1997;
                        endGfx = 1998;
                    } else if (NexAngelOfDeathWE.phase == 1) {
                        projectileId = 2002;
                        endGfx = 2003;
                    } else if (NexAngelOfDeathWE.phase == 2) {
                        projectileId = 2007;
                        endGfx = 2008;
                    }
                    npc.endGfx = endGfx;
            ...
            case 10402:
                ...
                if (chance > 1 && chance < 9) {
                    ...
                    int endGfx = -1;
                    int projectileId = -1;
                    if (ColossalHydra.phase == 0) {
                        projectileId = 2125;
                        endGfx = 2158;
                    } else if (ColossalHydra.phase == 1) {
                        projectileId = 2193;
                        endGfx = 2196;
                    } else if (ColossalHydra.phase == 2) {
                        projectileId = 2007;
                        endGfx = 2129;
                    }
                    npc.endGfx = endGfx;
            ...
            case 11401:
                if (npc.getAttackType() != null) switch (npc.getAttackType()) {
                    ...
                    case RANGE:
                        npc.setAttackType(CombatType.RANGE);
                        npc.endGfx = 2206; // start
                        npc.projectileId = 2237;
                        npc.maxHit = 75;
                        break;
                    case SPECIAL:
                        npc.setAttackType(CombatType.SPECIAL);
                        handler().groundAttack(npc, player, 2181 , 2207, 2260, 5);
                        npc.attackTimer = 8;
                        npc.hitDelayTimer = 5;
                        npc.endGfx = -1;
                        npc.projectileId = -1;
            ...
            case 11993:
                ...
                if (distanceToVet < 3) {
                    if (chance < 25) {
                        ...
                        npc.hitDelayTimer = 4;
                        npc.maxHit = 35;
                    } else if (chance > 90 && System.currentTimeMillis() - npc.lastSpecialAttack > 10000) {
                        npc.setAttackType(CombatType.SPECIAL);
                        npc.attackTimer = 5;
                        npc.hitDelayTimer = 2;
                        npc.lastSpecialAttack = System.currentTimeMillis();
                        handler().groundSpell(npc, player, 2344, 2346, "vetion", 2);
                        handler().groundSpell(npc, player, 2344, 2346, "vetion", 2);
                        handler().groundSpell(npc, player, 2344, 2346, "vetion", 2);
                        handler().groundSpell(npc, player, 2344, 2346, "vetion", 2);
                        handler().groundSpell(npc, player, 2344, 2346, "vetion", 2);
                    ...
                    if (chance < 71) {
                        ...
                        npc.hitDelayTimer = 4;
                        npc.maxHit = 35;
                    } else if (System.currentTimeMillis() - npc.lastSpecialAttack > 10000) {
                        npc.setAttackType(CombatType.SPECIAL);
                        npc.attackTimer = 5;
                        npc.hitDelayTimer = 2;
                        npc.lastSpecialAttack = System.currentTimeMillis();
            ...
            case 11994:
                chance = Misc.random(100);
                int distanceToVet1 = player.distanceToPoint(npc.absX, npc.absY);
                if (distanceToVet1 < 3) {
                    if (chance < 25) {
                        npc.setAttackType(CombatType.MAGE);
                        npc.attackTimer = 7;
                        ...
                        npc.hitDelayTimer = 2;
                        npc.lastSpecialAttack = System.currentTimeMillis();
                        handler().groundSpell(npc, player, 2345, 2347, "vetion", 2);
                        handler().groundSpell(npc, player, 2345, 2347, "vetion", 2);
                        handler().groundSpell(npc, player, 2345, 2347, "vetion", 2);
                        handler().groundSpell(npc, player, 2345, 2347, "vetion", 2);
                        handler().groundSpell(npc, player, 2345, 2347, "vetion", 2);
            ...
            case AvatarOfCreation.NPC_ID:
                int avatarofcreationRandom = Misc.random(100);
                if (avatarofcreationRandom <= 5) {
                    npc.setAttackType(CombatType.SPECIAL);
                    //npc.projectileId = avatarofcreation.SPECIAL_PROJECTILE;
                    npc.hitDelayTimer = 2;
                    npc.attackTimer = 10;
                    npc.maxHit = 3;
                    player.freezeTimer = 5;
                    player.gfx100(AvatarOfCreation.SPECIAL_HIT_GFX);
                    break;
                } else if (avatarofcreationRandom > 5 && avatarofcreationRandom <= 55) {
                    npc.setAttackType(CombatType.MAGE);
                    npc.projectileId = AvatarOfCreation.MAGIC_PROJECTILE;
                    npc.endGfx = 196;
                    npc.hitDelayTimer = 4;
                    npc.attackTimer = 10;
                    npc.maxHit = 3;
                    break;
                } else if (avatarofcreationRandom > 55) {
                    npc.setAttackType(CombatType.RANGE);
                    npc.projectileId = AvatarOfCreation.RANGE_PROJECTILE;
                    //npc.endGfx = 196;
                    npc.hitDelayTimer = 4;
                    npc.attackTimer = 10;
                    npc.maxHit = 3;
