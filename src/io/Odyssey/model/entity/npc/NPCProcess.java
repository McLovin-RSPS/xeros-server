...
public class NPCProcess {
    ...
    private void processing() {
        ...
        if ((!npc.underAttack) && !NPCHandler.isFightCaveNpc(npc) && !NPCHandler.isTokkulPit1Npc(npc) && npc.randomWalk && !npc.isDead() && npc.getBehaviour().isWalkHome()) {
            npc.facePlayer(0);
            npc.setPlayerAttackingIndex(0);
            // handleClipping(i);
            if (npc.spawnedBy == 0) {
    ...
    private void processDeath() {
        if (npc.isDead()) {
            ...
            if (npc.actionTimer == 0 && !npc.applyDead && !npc.needRespawn) {
                ...
                if (npc.getNpcId() == 11246 || npc.getNpcId() == 11670) {
                    System.out.println("NEX LOOT");
                    PlayerHandler.nonNullStream().filter(p -> npc.getNpcId() == 11670 ? Boundary.isIn(p, Boundary.angelOfDeathWorldevent) : Boundary.isIn(p, Boundary.REVENANT_MALEDICTUS))
                            .forEach(plr -> {
                ...
                switch (npc.getNpcId()) {
                    ...
                    case AvatarOfCreation.NPC_ID:
                        ...
                        break;
                    case 11670:
                        PlayerHandler.executeGlobalMessage("@red@[EVENT]@blu@ the world boss [@gre@AngelOfDeath@blu@] has been defeated!");
