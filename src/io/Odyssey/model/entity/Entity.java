...
import io.Odyssey.content.minigames.TombsOfAmascut.instance.TombsOfAmascutChestSupplies;
...
import io.Odyssey.content.minigames.xeric.Xeric;
...
import static io.Odyssey.model.entity.npc.NPCClipping.DIR;
...
public abstract class Entity {
        ...
        for (Entry<Entity, List<Damage>> entry : damageTaken.entrySet()) {
            ...
            if (tempKiller instanceof Player) {
                ...
                if (tempKiller.isPlayer()) {
                    Player p = tempKiller.asPlayer();
                    if (p.isDisconnected() || p.getSession() == null || !p.getSession().isActive()) continue;
                    if (this.getRaids3Instance() != null) {
                        if (p.getRaids3Instance() == null || p.getRaids3Instance() != this.getRaids3Instance()) continue;
                    }
                }
    ...
    public Entity calculateKiller() {
        ...
        for (Entry<Entity, List<Damage>> entry : damageTaken.entrySet()) {
            ...
            if (killer != null && killer instanceof Player && this instanceof NPC) {
                ...
                NPC npc = (NPC) this;
                if (player.getMode().isIronmanType() && !Boundary.isIn(player, Boundary.GODWARS_BOSSROOMS) && !Boundary.isIn(player, Boundary.CORPOREAL_BEAST_LAIR) && !Boundary.isIn(player, Boundary.DAGANNOTH_KINGS) && !Boundary.isIn(player, Boundary.TEKTON) && !Boundary.isIn(player, Boundary.SKELETAL_MYSTICS) && !Boundary.isIn(player, Boundary.RAIDS3_MAIN) && !Boundary.isIn(player, Boundary.RAID_MAIN)) {
    ...
    public Raids3 getRaids3Instance() {return raids3Instance; }

    public Entity setRaids3Instance(Raids3 raids3) {
        this.raids3Instance = raids3;
        return this;
    }
