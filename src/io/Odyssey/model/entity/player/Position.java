...
import io.Odyssey.model.collisionmap.Tile;
...
public final class Position {
	...
	public static Position of(int i, int i1, int i2) {
	   return null;
	}
	...
	public Position addX(int x) { return new Position(this.x + x, this.y); }
	public Position addY(int y) { return new Position(this.x, this.y + y); }
	...
	@JsonIgnore
	public boolean inRaids3Lobby() {//checks to see if player is in the raids3 lobby

		return Boundary.isIn(this, Boundary.RAIDS3_LOBBY);
	}
	@JsonIgnore
	public boolean inTombsOfAmascutLobby() {//checks to see if player is in the raids3 lobby

		return Boundary.isIn(this, Boundary.TOMBS_OF_AMASCUT_LOBBY);
	}
	@JsonIgnore
	...
	public boolean inMulti() {
		if (Boundary.isIn(this, Boundary.ZULRAH)
				...
				|| Boundary.isIn(this, Boundary.RAIDS3)
				|| Boundary.isIn(this, Boundary.TOMBS_OF_AMASCUT)
				...
				|| Boundary.isIn(this, Boundary.NexAngelOfDeath)
				|| Boundary.isIn(this, Boundary.ROCK_CRABS)
				|| Boundary.isIn(this, Boundary.SAND_CRABS)
				|| Boundary.isIn(this, Boundary.VETION_AREA)
		) {
			return true;
		}
	...
	// Tob
		if (Arrays.stream(TombsOfAmascutConstants.ALL_BOUNDARIES1).anyMatch(boundary -> Boundary.isIn(this, boundary))) {
			return true;
		}
		if(inRevs()) {
			return true;
