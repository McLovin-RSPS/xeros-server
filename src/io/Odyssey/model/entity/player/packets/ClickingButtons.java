...
import io.Odyssey.content.dialogue.DialogueActionButton;
import io.Odyssey.content.dialogue.DialogueConstants;
...
//				CityTeleports.teleport(c, actionButtonId);
		...
		case 57057:
			...
			break;
			case 183156:
				if (c.inDonatorBox == false && !c.getUltraInterface().isActive() && !c.getBlackAodInterface().isActive() && !c.getSuperBoxInterface().isActive() && !c.getNormalBoxInterface().isActive() && !c.getFoeInterface().isActive()) {
		...
		case 183155:
			if (!(c.getSuperMysteryBox().canMysteryBox) || !(c.getBlackAodLootChest().canMysteryBox) || !(c.getNormalMysteryBox().canMysteryBox) || !(c.getUltraMysteryBox().canMysteryBox) || !(c.getFoeMysteryBox().canMysteryBox)
			|| !(c.getYoutubeMysteryBox().canMysteryBox)) {
				c.getPA().showInterface(47000);
				c.sendMessage("@red@[WARNING] @blu@Please do not interrupt or you @red@WILL@blu@ lose items! @red@NO REFUNDS");
				break;
...
//			}
		...
		case 75010:
			...
			c.getPA().spellTeleport(3086, 3492, 0, true);
		...
		case 50056:
			...
			c.getPA().spellTeleport(3090, 3488, 0, true);
			break;
