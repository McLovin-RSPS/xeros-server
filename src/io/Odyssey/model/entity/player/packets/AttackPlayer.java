...
import io.Odyssey.content.combat.magic.LunarSpells;
		...
		case MAGE_PLAYER:
			...
			PlayerHandler.getOptionalPlayerByIndex(player.playerAttackingIndex).ifPresentOrElse(player1 -> {
				...
				if (player.getItems().isWearingItem(23900)) {
					player.crystalStaff -= 1;

				}
				if (player.crystalStaff < 1 && player.getItems().isWearingItem(23900)) {
					player.getItems().unequipItem(23900, 3);
					player.sendMessage("Your staff has ran out of charges.");
					player.attacking.reset();
				}
