...
import io.Odyssey.content.Sounds;
...
public class ItemAssistant {
			...
			if (ammoId != -1) {
				...
				if (ammoIsRangeAmmo) {
					if (player.getCombatItems().usingCrystalBow()
							...
							|| player.getCombatItems().usingWebweaverBow()
	...
	public boolean canEquip(int wearID) {
		...
		case 4587:
		case 20000:
			if (!Boundary.isIn(player, Boundary.LONE_SURVIVER_AREA) || !Boundary.isIn(player, Boundary.LUMBRIDGE_OUTLAST_AREA)) {
	...
	public boolean equipItem(int wearID, int slot) {
		...
		if (player.playerItems[slot] == (wearID + 1)) {
			...
			if(wearID == 20201 && player.crystalCharge < 1){
				player.sendMessage("This bow can be charge by crystal shard to be able to wear.");
				return false;

			}
			targetSlot = ItemStats.forId(item.getId()).getEquipment().getSlot();
			System.out.println("clicked item " + item.getId() + " slot:"+targetSlot
			);
			if (targetSlot == -1) {
	...
	public void updateBankContainer() {
		...
		player.getPA().sendConfig(1667, 0);
	...
	public void handleAmethyst() {
		if (!player.boltTips&&!player.arrowTips&&!player.javelinHeads) {
			...
			player.getItems().addItem(21338, 45);
